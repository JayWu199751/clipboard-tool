...
- ⬆️⬇️ 面板显示期间，**↑ / ↓ / Enter / Esc / Del / Z / 空格 被全局拦截**，只作用于面板，不会误输入到原程序（搜索模式下 Del / Z / 空格 让位给搜索输入框）
- 📌 **置顶**：点击条目上的置顶图标（或面板显示期间按 `Z`）把条目固定到列表最前；支持多条置顶、重启后保留，置顶条目优先保留、不受 200 条上限自动裁剪影响
- 🔍 **搜索**：面板内按 `空格` 进入搜索（搜索框常驻顶栏下方、未激活时灰色禁用）；支持按正文文字与来源应用（应用名/窗口标题）搜索、多词 AND 匹配、命中高亮；搜索时面板临时获得焦点以支持正常输入（含中文输入法），退出后立即归还
- 📌 面板固定在**鼠标所在屏幕正中间**弹出
...
> 面板是一个**不可激活（WS_EX_NOACTIVATE）**的置顶浮层：默认不抢走输入框焦点，全程键盘即可操作；仅按 `空格` 进入搜索时临时变为可聚焦以支持文字输入（含中文输入法），按 `Esc` 退出搜索后焦点立即归还原程序。
...
- 搜索模式下 `Space` / `Z` / `Del` 让位给搜索输入框（输入空格、打字、删字），`↑↓ / Enter / Esc` 仍操作面板；中文输入法组合期间（如候选词选择）面板导航键会暂时全部让位给输入法。
