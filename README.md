...
- 暗色描边四边等宽（2026-09-05 修复）：125% / 150% / 175% 各缩放档位下，暗色高亮描边四边宽度应一致、无一条边缺失或明显偏粗（四边等宽已由 `test:browser` 像素测量守住，实际合成与 DPI 仍需眼看；浅色发丝线边宽同批归一）。`.desktop` 已透明化、描边 100% 缩放下贴窗口边缘，贴边观感一并确认。
- 焦点恢复 + `Ctrl+V` 注入的实际时延。
