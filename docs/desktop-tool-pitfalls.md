...
- **分数缩放下贴着窗口物理边缘的描边会被裁掉，Chromium 又把边框宽度取整到整数设备像素**：透明面板的卡片描边压在窗口边缘的那条边会被 per-pixel alpha 裁淡乃至消失（125%/150% 下暗色描边只剩右/下两条边），所以外层容器要给描边留出内边距；但这只解决「有没有」，解决不了「等不等宽」——Chromium 把 border/inset box-shadow 的绘制矩形逐边取整到整数设备像素，1 CSS px 在 150% 下是 1.5 设备像素，四边各落在一或两个设备像素上，且调窄 CSS 宽度（0.667px）不改光栅。唯一稳定解是按 `resolution` 媒体查询把内边距与边宽同归一成 1/dppx，让盒边缘与描边宽度都恰为整数设备像素，取整无分数部分。测这类问题还有一坑：Playwright 截图默认垫白底（`omitBackground` 默认 false），半透明边缘全被垫亮，量出的「描边」其实是白垫透出——必须 `omitBackground: true`，并把卡片子元素整批隐藏（各子元素背景透明度不同，会按边不同程度地盖暗描边），再沿边积分覆盖率得出表观宽度。
